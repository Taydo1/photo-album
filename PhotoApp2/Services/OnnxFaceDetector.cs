...
namespace PhotoApp2.Services
{
    public class OnnxFaceDetector : IDisposable
    {
        ...
        public async Task<int> DetectFacesAsync(Mat inputMat)
        {
            ...
            return await Task.Run(() =>
            {
                try
                {
                    ...
                    Cv2.Resize(inputMat, resized, new Size(_inputWidth, _inputHeight));
                    ...
                    // Fast pixel normalization: (value - 127.0) / 128.0 for UltraFace RFB
                    for (int y = 0; y < _inputHeight; y++)
                    {
                        for (int x = 0; x < _inputWidth; x++)
                        {
                            Vec3b pixel = rgb.At<Vec3b>(y, x);
                            tensor[0, 0, y, x] = (pixel.Item0 - 127.0f) / 128.0f; // R
                            tensor[0, 1, y, x] = (pixel.Item1 - 127.0f) / 128.0f; // G
                            tensor[0, 2, y, x] = (pixel.Item2 - 127.0f) / 128.0f; // B
                        }
                    }
